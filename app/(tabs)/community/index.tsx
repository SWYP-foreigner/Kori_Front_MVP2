...
import { ActivityIndicator, FlatList, TextInput as RNTextInput, type ListRenderItem } from 'react-native';
...
type PostsListResp = {
    success: boolean;
    data: { items: PostsListItem[]; hasNext: boolean; nextCursor?: string };
    timestamp?: string;
};
...
const mapItem = (row: PostsListItem, respTimestamp?: string): PostEx => {
    ...
    const imageKeys: string[] =
        row.contentImageUrls ?? row.imageUrls ??
...
export default function CommunityScreen() {
    ...
    const handleToggleLike = async (postId: number) => {
        ...
        setItems(prev => prev.map(p => p.postId === postId ? { ...p, likes: Math.max(0, (p.likes ?? 0) + delta), likedByMe: !prevLiked } : p));
        try {
    ...
    const openSearch = () => {
        console.log('[community] openSearch');
        setSearchOpen(true);
        requestAnimationFrame(() => inputRef.current?.focus());
    };
    const closeSearch = () => { setQ(''); setSearchOpen(false); };
    ...
    const localMatches = useMemo(() => {
        ...
        return items.filter(p => `${p.author ?? ''} ${p.body ?? ''}`.toLowerCase().includes(kw));
    }, [items, canQuery, effectiveQ]);

    const composed: PostEx[] = useMemo(() => {
        if (!canQuery) return visible;
        const normalize = (x: PostEx | PostExFromSearch): PostEx => ({ ...x, postId: Number(x.postId), id: String(x.postId) });
        const primary = [...remoteHits.map(normalize), ...localMatches.map(normalize)];
        const seen = new Set<number>(); const out: PostEx[] = [];
        for (const p of [...primary, ...visible]) { if (!seen.has(p.postId)) { seen.add(p.postId); out.push(p); } }
        return out;
    ...
    return (
        <Safe>
            ...
            <List
                data={canQuery ? composed : visible}
...
const Header = styled.View`
  ...
  elevation: 2;
`;
const Left = styled.View<{ $grow?: boolean }>`
  flex-direction: row; align-items: center;
  ${({ $grow }) => ($grow ? 'flex: 1;' : '')}
`;
const Title = styled.Text`color: #ffffff; font-size: 32px; font-family: 'InstrumentSerif_400Regular'; letter-spacing: -0.2px;`;
const IconImage = styled.Image.attrs({ resizeMode: 'contain' })`margin-left: 4px; width: 20px; height: 20px;`;
...
const List = styled(FlatList as React.ComponentType<any>)``;
const FooterLoading = styled.View`padding: 16px 0;`;
const SearchBox = styled.View`flex: 1; height: 40px; background: #2a2b2c; border-radius: 10px; flex-direction: row; align-items: center; padding: 0 6px 0 12px; margin-right: 8px;`;
const Icon = styled.View`width: 20px; align-items: center; margin-right: 8px;`;
const ClearBtn = styled.Pressable`width: 28px; height: 28px; align-items: center; justify-content: center;`;
const CancelBtn = styled.Pressable`padding: 6px 8px;`;
const CancelText = styled.Text`color: #cfd4da;`;
