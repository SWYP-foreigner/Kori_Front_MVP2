...
const creator: StateCreator<PostUIState> = (set, get) => ({
  bookmarked: {},
  setBookmarked: (postId: number, value: boolean) => set((s) => ({ bookmarked: { ...s.bookmarked, [postId]: value } })),
  ...
  liked: {},
  likeCount: {},
  setLiked: (postId: number, liked: boolean) => set((s) => ({ liked: { ...s.liked, [postId]: liked } })),
  ...
  setLikeCount: (postId: number, count: number) =>
    set((s) => ({
      likeCount: { ...s.likeCount, [postId]: Math.max(0, count) },
    })),
  bumpLike: (postId: number, delta: number) => {
    const cur = get().likeCount[postId] ?? 0;
    set((s) => ({
      likeCount: { ...s.likeCount, [postId]: Math.max(0, cur + delta) },
    }));
  },
