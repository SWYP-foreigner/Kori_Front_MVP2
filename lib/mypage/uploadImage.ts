...
async function getPresignedForProfile(params: { filename: string; contentType: string }): Promise<PresignResp> {
  const { data } = await api.post<PresignResp>('/api/v1/member/profile/image/presign', {
